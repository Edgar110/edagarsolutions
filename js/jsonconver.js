...
return{
	Init:function(){
		if (window.File && window.FileReader && window.FileList && window.Blob) {
			...
			window.location = "http://edagarsolutions.com"
		}
	},
	load:function(event){
		readfile(event)
	...
	leer:function(){
			...
			for(i = 0;i < tamaño; i++){
				 ...
				 if(i == 0){$("#first").append("<th>"+variableName+"</th>")}
					html =  html + "<td><input type='text' value='"+valor+"'/></td>"
				 });
					html = html + "<td class='delete'><a onclick='del("+i+")' >X</a></td>"
				html = html + "</tr>"
			}
	...
	genera:function(){
		...
		jsonNew = [];
		titles = [];
		 tamañotit = $("#first th").length;
		for(i = 0;i < tamañotit; i++){
			var temp = $("#first th").eq(i).text()
			titles.push(temp)
		}
	...
	Agrega:function(){
		...
		htmlnew = htmlnew + "<td class='delete'><a onclick='del("+i+")' >X</a></td>"
		 htmlnew = htmlnew + "</tr>"
		$("#bod").append(htmlnew)
